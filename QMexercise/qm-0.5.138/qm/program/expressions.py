...
def math_func(name, *args):
    exp = _Q.AnyScalarExpression()

    exp.mathFunction.SetInParent()
    exp.mathFunction.loc = _get_loc()
    exp.mathFunction.functionName = name
    for arg in args:
        if isinstance(arg, _Q.ArrayVarRefExpression):
            element = exp.mathFunction.arguments.add()
