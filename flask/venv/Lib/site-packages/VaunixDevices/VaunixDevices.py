...
class VaunixDevices:
    ...
    def handleRequest(self, reqType, args):
        print(reqType, args)
        try:
            if reqType == 'resetManager':
                self.__init__()
                return 'success', self.devicesData
            elif reqType == 'initDevice':
                return self.initDevice(args['i'])
            elif reqType == 'setDeviceValue':
                self.setDeviceValue(args['key'], args['value'], args['i'])
                return self.getDeviceValues(args['i'])
            elif reqType == 'stopDevice':
