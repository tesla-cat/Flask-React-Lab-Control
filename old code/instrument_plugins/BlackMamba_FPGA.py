...
class BlackMamba_FPGA(Instrument):
    def __init__(self, name='BM'):
        ...
        self.add_function('get_card')
        self.add_function('get_cards')
        self.add_function('set_dump_path')
        self.add_function('load_tables')
        self.add_function('accept_stream')
        self.add_function('check_trigger')
    ...
    def stop(self, **kwargs):
        close_bm = kwargs.pop('close_bm', False)
        for c in self.get_cards():
            c.stop()
        if close_bm:
            self.bm.stop()
