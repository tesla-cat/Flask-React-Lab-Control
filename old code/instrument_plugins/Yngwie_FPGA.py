...
class Yngwie_FPGA(Instrument):
    ...
    def update_modes(self):
        ...
        if self.get_logic_version() == 'BXX':
            ...
            self.do_set_buffer_gen_width(10)

    def do_set_noutputs(self, v):
        self._noutputs = v

        # re-opening the card has the annoying side effect that the offsets
        # are being set to zero.
        offsets = {}
        nmodes = self.get_nmodes()
        for imode in range(nmodes):
            offsets[imode] = self.get('offset%d' % imode)

        self.open()

        for imode in range(nmodes):
            self.set('offset%d' % imode, offsets[imode])
