...
class Remote_Instrument(Instrument):
    ...
    def __init__(self, name, remote_name=None, remote_srv=None, remote_inssrv_name=None):
        super(Remote_Instrument, self).__init__(name)

        self._remote_name = remote_name
        self._remote_srv = remote_srv
        self._remote_inssrv_name = remote_inssrv_name

        objsh.backend.connect_to(self._remote_srv)
        self._srv = objsh.find_object(self._remote_inssrv_name)
        self._remote_ins = self._srv[self._remote_name]

        params = self._remote_ins.get_pickleable_parameters()
        funcs = self._remote_ins.get_functions()
    ...
    def _get(self, channel, **kwargs):
        return self._remote_ins.get(channel, **kwargs)

    def _set(self, val, channel, **kwargs):
        return self._remote_ins.set(channel, val, **kwargs)

    def _call(self, funcname, *args, **kwargs):
        return self._remote_ins.call(funcname, *args, **kwargs)
